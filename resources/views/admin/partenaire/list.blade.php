...
@section('content')
    ...
    <div class="uk-width-1-2@l uk-width-1-1@m uk-width-1-1@s" >
        ...
        <h4 class="rajdhani uk-margin-remove">Liste des partenaires</h4>
    ...
    <script type="text/javascript">
        ...
        xhr.open('GET', '{{ route('read', 'partenaire')  }}');
                    ...
                    if(arr.errorState === 'OK'){
                        ...
                        for(let i=0; i < data.length; i++){
                            str += '<div class="uk-card uk-card-default uk-width-1-2@l uk-width-1-2@m uk-width-1-1@m" style="border: 1px solid #CCC; margin: 8px; padding: 0;">\n' +
                                ...
                                '                                <div class="">\n' +
                                '                                    <img class="" style="max-width: 100%; display: block; margin: auto;" src="{{ asset('partenaire') }}/'+data[i].logo+'">\n' +
                                '                                </div>\n' +
                                '                            </div>\n' +
                                '                        </div>\n' +
                                '                        <div class="uk-card-body" style="overflow: hidden; padding: 15px;">\n' +
                                '                            <a href="'+data[i].site_web+'" target="_blank">'+data[i].site_web+'</a>\n' +
                                '                        </div>\n' +
                                '                        <div class="uk-card-footer" style="padding: 5px 15px;">\n' +
                                '                            \n' +
                                '                            <a href="{{ route('delete', ['item' => 'patenaire', 'id' => '']) }}/'+data[i].id+'" uk-tooltip="title: Supprimer; pos: bottom" class="uk-text-danger uk-float-right">\n' +
                                '                                <span uk-icon="icon: trash"></span>\n' +
                                '                            </a>\n' +
                                '                            <a href="{{ route('update', ['item' => 'partenaire', 'id' => '']) }}/'+data[i].id+'" uk-tooltip="title: Modifier; pos: bottom" class="uk-float-right" style="margin-right: 10px;">\n' +
